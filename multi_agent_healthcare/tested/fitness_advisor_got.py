...
# Updated with Evaluation Metrics/Scores and Bias Mitigation
...
# ================================
# Helper Function for Response Scoring
# ================================
def compute_response_score(text1: str, text2: str) -> float:
    emb1 = embedder.embed_documents([text1])[0]
    emb2 = embedder.embed_documents([text2])[0]
    # Cosine similarity
    return np.dot(emb1, emb2) / (np.linalg.norm(emb1) * np.linalg.norm(emb2))

# ================================
# 4. Doctor Node (Updated with Retrieval Scores)
# ================================
def doctor_node(state: AgentState):
    results = collection.query(
        query_texts=[state['user_question'] + " fitness exercise health impact"],
        n_results=6,
        include=["documents", "distances", "metadatas"]  # Add distances
    )
   
    context_blocks = []
    scores = []  # New: Store similarity scores
    for i in range(len(results['documents'][0])):
        doc = results['documents'][0][i]
        pmid = results['ids'][0][i]
        dist = results['distances'][0][i]
        sim_score = 1 - (dist ** 2) / 2  # Approx cosine sim for unit vectors
        scores.append(sim_score)
        context_blocks.append(f"[PMID: {pmid}] (Similarity: {sim_score:.2f}) {doc.strip()[:1200]}...")
...
DOCTOR: [your response with real PMID citations]"""
    ...
    sources_str = " | ".join([f"[bold cyan]PMID:{pid} (Sim: {scores[j]:.2f})[/bold cyan]" for j, pid in enumerate(results['ids'][0][:4])])
    console.print(Panel(resp.content + f"\n\nReal sources → {sources_str}\nAvg Relevance Score: {avg_score:.2f} (higher = better match to query, explains why these were chosen)",
                        title="[blue]Doctor – 272k Real PubMed[/blue]", border_style="blue"))
   
    # Compute response score
    query_combined = state['user_question'] + " " + state['user_profile']
    response_score = compute_response_score(query_combined, resp.content)
    console.print(f"Doctor Response Relevance Score: {response_score:.2f} (explains alignment to user goal)")
    ...
    thought = {
        ...
        "relevance_score": avg_score,  # Retrieval score
        "response_score": response_score  # Generation score
    }
...
# ================================
# 5. Critic Node (Updated with Response Score)
# ================================
...
CRITIC: """
    ...
    query_combined = state['user_question'] + " " + state['user_profile']
    response_score = compute_response_score(query_combined, resp.content)
    console.print(f"Critic Response Relevance Score: {response_score:.2f} (explains alignment to user goal)")
...
# ================================
# 6. Supporter Node (Updated with Response Score)
# ================================
...
SUPPORTER: """
    ...
    query_combined = state['user_question'] + " " + state['user_profile']
    response_score = compute_response_score(query_combined, resp.content)
    console.print(f"Supporter Response Relevance Score: {response_score:.2f} (explains alignment to user goal)")
...
# ================================
# 7. Bias Checker Node (New for Bias Mitigation)
# ================================
def bias_checker_node(state: AgentState):
    debate = "\n\n".join([t['content'] for t in state["thoughts"]])
    prompt = f"""Scan for biases (e.g., gender, age, cultural, overgeneralization).
    Debate: {debate}
    User: {state['user_profile']}
    List any issues and suggest fixes.
    BIAS CHECK: """
    
    resp = critic_llm.invoke(prompt)
    #console.print(Panel(resp.content, title="[orange]Bias Checker[/orange]", border_style="orange"))
    console.print(Panel(resp.content, title="[orange3]Bias Checker[/orange3]", border_style="orange3"))
    
    # Compute response score (optional for checker)
    query_combined = state['user_question'] + " " + state['user_profile']
    response_score = compute_response_score(query_combined, resp.content)
    console.print(f"Bias Checker Response Relevance Score: {response_score:.2f}")
    ...
    save_agent_message("BiasChecker", resp.content)
    
    return {"thoughts": state["thoughts"] + [thought]}

# ================================
# 8. Summary Node (Updated with Aggregate Scores)
# ================================
def summary_node(state: AgentState):
    debate = "\n\n".join([f"{t['agent']}: {t['content'][:400]}" for t in state["thoughts"]])
    
    # Aggregate scores
    all_relevance_scores = [t.get("relevance_score", 0) for t in state["thoughts"]]
    all_response_scores = [t.get("response_score", 0) for t in state["thoughts"]]
    avg_relevance = sum(all_relevance_scores) / len(all_relevance_scores) if all_relevance_scores else 0
    avg_response = sum(all_response_scores) / len(all_response_scores) if all_response_scores else 0
    avg_debate_score = (avg_relevance + avg_response) / 2
...
Include overall debate score: {avg_debate_score:.2f}/2.0 (higher = stronger evidence and relevance).
Debate:
{debate}
...
"""
    ...
    console.print(Panel(summary_text + f"\nDebate Quality Score: {avg_debate_score:.2f} (justifies why this evidence set was chosen over regeneration)",
                        title="[bold magenta]Evidence Debate Summary (150 words max)[/bold magenta]", border_style="magenta"))
...
# ================================
# 9. Coach Node (Updated with Response Score)
# ================================
...
"""
    ...
    query_combined = state['user_question'] + " " + state['user_profile']
    response_score = compute_response_score(query_combined, resp.content)
    console.print(f"Coach Response Relevance Score: {response_score:.2f} (explains alignment to user goal)")
...
# ================================
# 10. Build Graph-of-Thoughts (Updated with Bias Checker)
# ================================
...
graph.add_node("bias_checker", bias_checker_node)
...
graph.add_edge("supporter", "bias_checker")
graph.add_edge("bias_checker", "summary")
